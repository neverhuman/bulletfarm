...
fn lease_socket_refuses_before_startup_without_registered_peer_configuration() {
    ...
    assert!(admit_lease_transport(Some(missing.clone()), None, None, Some(&registration)).is_err());
    assert!(
        !missing.exists(),
    ...
    .expect("registry");
    let durable = admit_lease_transport(Some(socket.clone()), Some(&registry), Some(&key), None)
...
fn worker_token_file_is_regular_private_and_single_line() {
    ...
    std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o600)).expect("private mode");
    assert_eq!(read_worker_token(&path).expect("read"), token);

    std::fs::set_permissions(&path, std::fs::Permissions::from_mode(0o640)).expect("group mode");
