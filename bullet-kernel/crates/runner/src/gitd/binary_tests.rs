...
async fn invalid_subjects_never_execute_canary() {
    ...
    for result in [
        ...
        assert_refused_without_execution(result, &marker, "GITD_BINARY_ADMISSION_REFUSED").await;
    }
...
async fn sealed_image_survives_same_inode_overwrite() {
    ...
    let mut session =
        crate::gitd::GitdSession::spawn_with(admitted, [marker.as_os_str()], serde_json::json!({}))
            .await
