fn verify_schema_twenty_two_refusals() {
    if let Some(path) = std::env::var_os("BULLET_SCHEMA_PREFIX_WAL_CHILD") {
        ...
        let version = if mode.starts_with("27") {
            27
        } else if mode.starts_with("26") {
            26
...
fn assert_typed_prefix_inspection(
    ...
    if expected == "UPGRADE_REQUIRED" {
        ...
        assert_eq!(
            schema.schema_state(),
            super::SchemaState::UpgradeRequired { from: 22, to: 27 }
        );
...
fn verify_crashed_schema(mode: &str) {
    ...
    for _ in 0..2 {
        ...
        if mode.starts_with("27") && !mode.contains("super") {
