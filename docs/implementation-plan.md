# BulletFarm: one workbench, one controller, one verified delivery path

The exhaustive current-state, tips reconciliation, BF3-001–030 acceptance/scenario matrix,
...
the earlier private dashboard/pruning direction. The sole repository is
`neverhuman/bulletfarm`, with canonical checkout `/home/ubuntu/bulletfarm`.
The [consolidation plan](migration/README.md) lands before controller implementation.
The documented command becomes `bulletfarm`, retaining `bf` as an alias and preserving
`~/.bf` / `BF_DATA_DIR`. Quoted earlier instructions below retain their original wording.
...
journey. The product remains `neverhuman/bulletfarm`: one Rust package with embedded React/TypeScript/Vite.
Preserve legacy BulletFarm source and evidence in archival refs before retiring their repositories.
...
`neverhuman/bulletfarm`; Cursor second; TUI optional on shared API. Product boundary is approved goal → bounded
...
Typing `bulletfarm` (or its `bf` alias) opens installed workbench, reconnects selected hub, restores project/conversation/draft/work
...
inputs under separate authority. For BulletFarm, authoritative behavioral test is external browser harness:
...
In `neverhuman/bulletfarm`: **Add work-list pagination so users can reach older tasks without losing current
