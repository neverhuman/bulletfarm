...
| Offline five-plane proof | `COMPONENT-COMPLETE` / catalog `BLOCKED` | `just demo` now writes a signed `TRANSACTION_PROOF` (fences `1→2`, fixture gitd, writer-overlap E2 refuse, lost-response `UNKNOWN` then identity-exact adopt, preserve, stale fence refuse, command `pending` not empty success). Production gitd still refuse. Protected live integration is not this receipt. Do not hand-edit `release-truth.generated.md`; `check release` must keep the other 25 gates `BLOCKED` |
