...
This follows the [full-product plan](full-product-dogfood-plan.md), [G1–G18 register](product-gaps.md) and typed profile inventory. The [health checkpoint](health-checkpoint-20260909.md) records accepted subjects and retained failures. Those authorities and the existing DF/WP identifiers remain unchanged; this document supplies the detailed execution and verification order.

## Current evidence and immediate corrections

The [gap register](product-gaps.md) and [first measured cycle](deep-audit-20260911.md#first-measured-implementation-cycle) retain exact current subjects, accepted checks, failed runs, proposals and artifact availability, including the 159-file baseline and 18-source/12-evidence capsule. Recover missing originals or rerun into durable storage; fragments cannot reconstruct acceptance. Preserve rejected bytes/stashes; unknown failure causes remain unknown. H1/H2 require current member/family/native-audit/hosted/aggregate proof; U1 also needs actual current Tuiwright/Playwright and installed behavior. M1 retains blank/stripe/ one-frame/wrong-subject failures. R1 and every profile await semantic admission. The 165 findings / 28 cap occurrences remain triage; each disposition needs exact source, fingerprint, reproducer, review and result. Continuously monitor changed-and-restored source, overflow and tool/config/lookup drift through compilation/final read-back; ending hashes alone cannot prove custody. Gate admission precedes launch. Preserve zero-spawn versus post-spawn failure, uncertain cost, nonzero exit, timeout and missing completion separately: process spawn, CLI exit and billed turn are distinct observations.

The September 12 continuation implements the exact-request portion of U1a.1 in Kernel `apps/bullet/src/coding/journal.rs`, `coding/http.rs` and `coding.rs`. New schema-2 journals retain the serialized UTF-8 request and dispatch those bytes directly. Historical schema-1 records remain unchanged and recovery is explicitly labelled `RECONSTRUCTED_V1`; their original serialization cannot be recovered. Focused component tests cover noncanonical JSON/Unicode bytes on the actual HTTP connection, fresh persistence before networking, response loss, session refusal, strict corruption/schema rejection, restart and maximum valid task storage bounds. The corrected local Kernel checkpoint `4053b2d` passes its complete canonical `required` lane: 1280 selected fast tests, 34 contract tests, five preservation policy checks, lint, security and documentation. The earlier `ff3c121` run passed 1280 selected tests then failed strict lint; its failure is retained separately. The same-source compiled CLI passes all 12 existing Tuiwright component scenarios; 127 emitted artifact hashes and 13 JUnit rows (12 scenarios plus run integrity) were independently checked against selection, completion and process status. These are component observations only. Owner-bound drafts, atomic revision and settlement/completion relationships, and authenticated absence before replay remain open; this packet does not complete U1a.1 or authorize a composer.

H1/H2 continuation in Kernel `4ab8885` implements the explicit `hosted-component` profile, required `operator-tui` workflow job, both source-built aliases and seven-job aggregate consumer; `1ac32cb` repairs a shell fixture that inherited unrelated Cargo custody. The aggregate compiles its trusted validator from current source rather than executing downloaded artifacts. Validation binds source/executable/harness/workflow/run/attempt, exact selected/completed scenarios, emitted artifact hashes, JUnit agreement and cleanup. The profile passes 56 route/refusal controls, including 17 direct native-validator refusals; four JUnit and seven consumer tests pass. These are implementation/component results, not actual hosted execution. The new bare-alias scenario brings enumeration to 13; it proves the existing shell/account-required behavior, not onboarding or a goal editor. The pinned Tuiwright reader exposes counts and rendered screens but no raw-output capture API; raw transcripts, the full required user journeys, bounded performance workloads and real-provider qualification remain open.

Reviewed reconciliation `a809f03` preserves both parent histories and the authenticated journal/containment boundaries; incoming fabricated task identities and unsupervised execution were rejected. Pagination/history enhancements remain deferred pending owner/identity controls. Adapted private HOME/XDG demo defaults pass four tests executing both aliases, strict focused lint and independent review; the inventory at that checkpoint was 1330 total / 1284 standalone. The earlier `4ab8885` required run passed 1280 selected fast tests, Rust lint and five preservation policy checks, then failed the new raw-source shell fixture; contract/security/docs did not execute. Its originals remain distinct from the corrected source. The `a809f03` required run then failed the unchanged 40-second fast timeout: 15 passed and two batched demo tests timed out, with 1267 selected tests and all later lanes unexecuted. Splitting each demo invocation into its own bounded case is the repair; this failure remains retained. Correction `c036765` splits eight positive alias/default demo invocations into separate tests; its complete required lane passes 1290 selected fast tests, 34 contract tests, five preservation checks, lint, security and docs, with the full 1336-test inventory verified. A later bare-launch PTY failure exposed an inherited fixture-home mode before product execution (12 scenarios passed, one failed); `f8a2b5c` explicitly creates that fixture as 0700 without weakening its launcher. Both default aliases then pass all 13 component scenarios under umask 002, with 128 artifact hashes and 14 JUnit cases checked per run; all 11 harness tests pass. The `f8a2b5c` required run passed tests/lint/security but failed the unchanged 25-commit documentation freshness limit at distance 26. Independent re-review corrected stale README demo/CI claims and refreshed four source markers in `191ab0a`; its complete required rerun passes 1290 fast, 34 contract and five preservation checks, lint, security and documentation with final exit zero and released custody. GitHub refused the workflow-changing branch update because the OAuth credential lacks `workflow` scope, and the available SSH identity has read-only permission; no hosted run or remote update followed. Portal now has a reviewed preparation and explicitly unusable admission proposal binding 49 tools, 365 input roots, 819 lookups, five fixed fixture subjects and a fresh monitor. Four private runtime directories, exact tool links, eleven version observations/six required pins and configuration absences are verified. Actual remaining writer releases, sustained freeze/mount conditions and final completeness/admission approval remain missing; no selector or proof was issued. Historical b64/container records cannot admit current `44a2ccd`; its required lane remains unexecuted. Hosted Portal policy/tool-profile/topology admission is separate.
