...
| G2 | Connected five-plane transaction | Family `check release` still reports `release.transaction-demo` `BLOCKED` (do not hand-edit `release-truth.generated.md`). The offline component receipt exists: `just demo` writes a signed `TRANSACTION_PROOF` (fences `1→2`, fixture gitd child, writer-overlap E2 refuse, lost-response `UNKNOWN` then identity-exact adopt, stale fence refuse, command stays `pending`). Production `bullet-gitd` `clone` remains `AUTHORITY_CONTRACT_UNAVAILABLE`. Protected live integration and packaged Portal are not this receipt | Signed farmd-internal lease transport; `bullet-gitd-fixture` feature/bin; LocalBareForge adopt; Portal `PENDING→UNKNOWN` | Orchestrator commits T3 subjects and admits the catalog row; keep the other 25 gates `BLOCKED` | [`v1-closure-plan.md`](v1-closure-plan.md) V1-S4 |
...
| V1-S4 | COMPONENT-COMPLETE; family catalog still BLOCKED | Credential-free signed `TRANSACTION_PROOF` from `just demo`; production gitd still refuse; `release.transaction-demo` stays `BLOCKED` until generated truth admits the receipt |
