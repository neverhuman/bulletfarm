...
async fn strict_envelope_and_authenticated_status_reads_refuse_ambiguity() {
    ...
    assert!(connection.execute(
        "DELETE FROM events WHERE kind = 'command_submitted' AND body = ?1 AND correlation_id = ?1",
        params![admitted_id],
    ).is_err(), "ownership FK preserves submitted audit truth");
    // Deliberately damage this isolated fixture after proving normal deletion
    // is refused. The original corrupt-audit response assertion remains below.
    connection
        .pragma_update(None, "foreign_keys", "OFF")
        .expect("corruption fixture only");
    connection
        .execute(
            "DELETE FROM events
...
include!("command_auth/reconciliation.rs");
