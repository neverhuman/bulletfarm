...
| [`loopback-console.md`](loopback-console.md) | unsigned loopback farmd + TUI + Portal + worker-loop from a bulletfarm clone | `just console`, no-args `bullet` after login, HOLD-honest TUI/Portal; worker can claim and typed-refuse | signed install; Head Send; production coding settlement; live turns without enrollments; HOLD lift; VERIFIED |
...
| [`dogfood-admission-kit.md`](dogfood-admission-kit.md) | **PROPOSED, not executable** — future operator acts for the dogfood track and its purpose-separated, non-evidence `DOGFOOD_RUN` operational observation | state exactly what is and is not true today, and what must still refuse | be executed: the typed Hub commands, exact-model enrollment and credential projection, filesystem containment, dogfood-scoped binding, bounded budgets, canonical policy producer/read-back, durable incident inventory, exact four-repository W0 drift refusal, and operational record are unimplemented; OD-K's predecessors and fresh safety review are unmet |
