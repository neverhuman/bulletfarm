#!/usr/bin/env bash
# Closed Hub and BulletGit source-scan adapters. Unsigned diagnostics only.
...
publication_tools() {
  ...
  for name in awk bash basename cat chmod cmp cp cut df dirname env find git grep head id jq mkdir mv \
...
# Exactly two reviewed source-scan profiles; no caller-provided paths or argv.
publication_profile() {
  PUBLICATION_MEMBER="$1"
  case "$PUBLICATION_MEMBER" in
    bullet-farm)
      PUBLICATION_JOB=publication_integrity
      PUBLICATION_STEM=bullet-publication-member
      PUBLICATION_PURPOSE=BOOTSTRAP_MEMBER_DIAGNOSTIC_VALIDATION
      PUBLICATION_SCHEMA=bullet.publication-bootstrap-source-scan.v1
      PUBLICATION_SCOPE=BOOTSTRAP_PLUS_HUB_SOURCE_SCAN
      PUBLICATION_OUTPUT=member_completion_sha256 ;;
    bullet-git)
      PUBLICATION_JOB=bullet_git_source_scan
      PUBLICATION_STEM=bullet-publication-git-member
      PUBLICATION_PURPOSE=MEMBER_DIAGNOSTIC_VALIDATION
      PUBLICATION_SCHEMA=bullet.publication-member-source-scan.v1
      PUBLICATION_SCOPE=GIT_SOURCE_SCAN
      PUBLICATION_OUTPUT=git_member_completion_sha256 ;;
    *) publication_refuse PUBLICATION_CI_JOB_ADAPTER_UNSUPPORTED; return 1 ;;
  esac
  PUBLICATION_KEY="$PUBLICATION_MEMBER:REQUIRED:source_scan"
}

publication_member() {
  publication_profile "${1:-bullet-farm}"
  publication_host "$PUBLICATION_JOB"
  umask 077
  local family="$RUNNER_TEMP/bullet-publication-family" binary="$RUNNER_TEMP/bullet-publication-target/debug/bullet-publish"
  local destination="$RUNNER_TEMP/$PUBLICATION_STEM-report" scratch="$RUNNER_TEMP/$PUBLICATION_STEM-private"
  ...
  mkdir "$destination"
  "$binary" ci-job-context "$GITHUB_WORKSPACE" "$family" "$PUBLICATION_KEY" \
  ...
  publication_run_scan "$family/$PUBLICATION_MEMBER" "$commit" "$scratch" "$scanner"
  mkdir "$destination/member"
  if [[ "$PUBLICATION_MEMBER" == bullet-git ]]; then
    cp -R "$family/bullet-git/target/ci-upload/source-scan" "$destination/member/.ci-artifacts"
  else
    cp -R "$family/bullet-farm/.ci-upload/source-scan/.ci-artifacts" "$destination/member/"
  fi
  "$binary" ci-job-observe "$GITHUB_WORKSPACE" "$family" "$PUBLICATION_KEY" \
  ...
  jq -n --arg context "$(publication_hash "$destination/context.json")" \
    ...
    --arg schema "$PUBLICATION_SCHEMA" --arg scope "$PUBLICATION_SCOPE" '
  ...
  publication_validate_member "$destination" "$completion" "$PUBLICATION_MEMBER"
  publication_host "$PUBLICATION_JOB"
  [[ -f "${GITHUB_OUTPUT:-}" && ! -L "$GITHUB_OUTPUT" ]] \
    || { publication_refuse PUBLICATION_MEMBER_OUTPUT_INVALID; return 1; }
  printf '%s=%s\n' "$PUBLICATION_OUTPUT" "$completion" >>"$GITHUB_OUTPUT"
}

publication_validate_member() {
  publication_profile "${3:-bullet-farm}" || return 1
  ...
  jq -e --slurpfile c "$context" --slurpfile v "$validation" --slurpfile o "$observation" \
    ...
    --arg validator "$(publication_hash "$GITHUB_WORKSPACE/$PUBLICATION_MEMBER/ops/ci/artifact-check.sh")" \
    --arg member "$PUBLICATION_MEMBER" --arg key "$PUBLICATION_KEY" --arg job "$PUBLICATION_JOB" \
    --arg purpose "$PUBLICATION_PURPOSE" --arg schema "$PUBLICATION_SCHEMA" --arg scope "$PUBLICATION_SCOPE" \
    ...
    --arg nested_workflow "$(publication_hash "$GITHUB_WORKSPACE/$PUBLICATION_MEMBER/.github/workflows/ci.yml")" '
    ...
    .schema_version == $schema and .scope == $scope and
    ...
    (.tool_subjects|keys_are(["awk","bash","basename","cat","chmod","cmp","cp","cut","df","dirname","env","find","git","grep","head","id",
    ...
    $c[0].purpose == $purpose and $c[0].execution_evidence == false and
    $c[0].hosted == {event_sha:$event,event_name:$name,run_id:$run,run_attempt:$attempt,
      workflow_ref:$ref,workflow_sha:$workflow,job:$job} and
    ...
    $c[0].subject.invocation_key == $key and
    $c[0].subject.invocation == {member:$member,member_commit:$m[0].members[$member].commit,
      member_tree:$m[0].members[$member].tree,workflow_path:".github/workflows/ci.yml",workflow_sha256:$nested_workflow,
    ...
    $o[0].schema_version == "bullet.ci-observation.v1" and $o[0].repository == $member and
    $o[0].signed == false and $o[0].evidence_class == "DIAGNOSTIC_ONLY" and
    $o[0].tool_versions.gitleaks == "8.21.2" and (if $member == "bullet-farm" then ($o[0].tool_versions.python|test("^Python 3[.]12[.][0-9]+$")) else true end) and
    ($o[0].tool_versions.git|startswith("git version ")) and
    $o[0].commit_oid == $m[0].members[$member].commit and $o[0].tree_oid == $m[0].members[$member].tree and
...
# Standalone adapter consumer; wiring it into the all-job gate is a later packet.
publication_git_required() {
  publication_host publication_required
  [[ "${BULLET_GIT_SOURCE_SCAN_RESULT:-}" == success ]] \
    || { publication_refuse PUBLICATION_GIT_SOURCE_SCAN_DID_NOT_SUCCEED; return 1; }
  local root="$RUNNER_TEMP/bullet-publication-git-downloaded"
  local member="publication-git-source-scan-$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT"
  publication_directory "$root"
  publication_inventory "$root" "d $member" 1 \
    || { publication_refuse PUBLICATION_REQUIRED_UPLOAD_INVENTORY; return 1; }
  publication_validate_member "$root/$member" "${BULLET_GIT_MEMBER_COMPLETION_SHA256:-}" bullet-git
  publication_host publication_required
  printf 'one BulletGit source scan passed; complete applicable CI remains required\n'
}
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  case "$1" in
    ...
    git-member-run) publication_member bullet-git ;;
    git-required) publication_git_required ;;
