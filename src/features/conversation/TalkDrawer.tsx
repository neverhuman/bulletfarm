  ...
  return (
    <aside className="talk-panel" aria-labelledby={titleId}>
      <header className="talk-head">
        ...
        <p className="talk-tag">Schema-27 conversation_message overlay · not a 16th surface · not G15 cognition</p>
      ...
      <div className="talk-thread">
        {index !== null && index.conversations.length > 0 ? (
          <label className="talk-label" htmlFor="talk-thread-select">
            Owned threads
            <select
        ...
        {thread === null ? (
          <p className="talk-cite">
            GET /api/v1/conversations is the thread of record. No local transcript. Assistant rows require a native
            Head outcome.
          </p>
        ) : null}
        {cite ? <p className="talk-cite">{cite}</p> : null}
        {indexIncomplete ? <p className="talk-refuse">CONVERSATION_INDEX_INCOMPLETE: more owned threads remain after this snapshot page</p> : null}
        {pageIncomplete ? <p className="talk-refuse">CONVERSATION_PAGE_INCOMPLETE: composer waits until the current snapshot page reaches the tip</p> : null}
        <p className="talk-refuse" data-testid="talk-blocker">
          {thread?.head_blocker ?? HEAD_RUNTIME_BINDING_REQUIRED}
        </p>
        {error ? (
          <p className="talk-refuse" data-testid="talk-error">
            {error}
          </p>
        ) : null}
        <p className="talk-cite">{SLACK_BIND_UNAVAILABLE}</p>
        <p className="talk-cite">{TELEGRAM_UNAVAILABLE}</p>
      </div>
      <form className="talk-form" onSubmit={submit}>
        <label className="talk-label" htmlFor="talk-line">
          Message the Head
        </label>
        <textarea
          id="talk-line"
...
const css = `
.talk-root { position: fixed; right: 20px; bottom: 20px; z-index: 20; }
.talk-launch {
  border-radius: 999px;
  box-shadow: 0 0 0 1px #2a3a55, 0 12px 40px #05070f;
...
.talk-panel {
  ...
  background: #101828;
  box-shadow: inset 0 0 0 1px #2a3a55, 0 24px 80px #05070f;
  padding: 20px;
  gap: 16px;
}
.talk-title { margin: 0; font-size: 22px; font-weight: 800; }
.talk-tag { margin: 6px 0 0; color: #c5d4ea; }
.talk-chips { display: flex; flex-wrap: wrap; gap: 8px; margin-top: 12px; }
.talk-thread { overflow: auto; display: grid; gap: 12px; }
.talk-user, .talk-assistant, .talk-cite, .talk-refuse { margin: 0; white-space: pre-wrap; }
.talk-user { color: #a9c7ff; }
.talk-assistant { color: #f4f7ff; }
.talk-cite { color: #9db0c9; font-size: 12px; }
.talk-refuse { color: #ffc44d; font-weight: 700; }
.talk-form { display: grid; gap: 8px; }
.talk-label { font-size: 13px; color: #c5d4ea; display: grid; gap: 6px; }
.talk-input { width: 100%; resize: vertical; }
`;
